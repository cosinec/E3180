namespace ConsoleApplication1
{
    ...
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            // 设置端口对象的属性
            _serialPort.PortName = SetPortName(_serialPort.PortName);//端口
            _serialPort.BaudRate = SetPortBaudRate(_serialPort.BaudRate);//波特率
            _serialPort.Parity = SetPortParity(_serialPort.Parity);//奇偶值
            _serialPort.DataBits = SetPortDataBits(_serialPort.DataBits);//数据位
            _serialPort.StopBits = SetPortStopBits(_serialPort.StopBits);//停止位
